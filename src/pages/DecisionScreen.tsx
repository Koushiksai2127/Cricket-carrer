          ...
          <div className="grid gap-8">
            <AnimatePresence>
              {DECISIONS.map((decision, idx) => (
                ...
                >
                  <Card className="bg-white border-slate-200 rounded-[2.5rem] shadow-xl shadow-slate-200/50 overflow-hidden group hover:shadow-2xl transition-all duration-500">
                    <div className="bg-slate-50 px-10 py-5 border-b border-slate-100 flex justify-between items-center">
                      ...
                      {(() => {
                        ...
                        return <DecisionIcon className={`w-5 h-5 ${decision.color} transition-colors`} />;
                      })()}
                    </div>
                    <CardContent className="p-10">
                      <div className="mb-10">
                        <h2 className="text-3xl font-black text-slate-900 mb-3 tracking-tight">{decision.title}</h2>
                        <p className="text-slate-500 leading-relaxed text-lg">{decision.description}</p>
                      </div>
                        ...
                        {decision.options.map((option) => {
                          ...
                          return (
                            <Button
                              ...
                              className="h-auto py-8 px-6 flex flex-col items-center text-center space-y-4 rounded-3xl border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all group/btn relative overflow-hidden"
                            >
                              <div className="p-4 bg-slate-100 rounded-2xl group-hover/btn:bg-blue-100 transition-colors">
                                <Icon className="w-8 h-8 text-slate-600 group-hover/btn:text-blue-600" />
                              </div>
                              <span className="font-black text-sm text-slate-700 group-hover/btn:text-blue-700 uppercase tracking-wider">{option.text}</span>
                              <div className="absolute bottom-0 left-0 w-full h-1 bg-blue-600 scale-x-0 group-hover/btn:scale-x-100 transition-transform origin-left" />
                            </Button>
                          );
                        })}
                      </div>
